...
public class StructureMapSingleton {
	...
	private StructureMapSingleton(StructureMap inRootMap,List<StructureMap> inChildMaps, String inObjectId) {
		rootMap = inRootMap;
	...
	public static StructureMapSingleton getAndUpdateInstance(final StructureMapDTO structuresMapDTO , final String objectId) {
		if(mapInstance != null) {
		...
		synchronized(StructureMapSingleton.class) {
			if (instance == null) {
				try { 
					...
					instance = new StructureMapSingleton(rootMap, childMaps,objectId);
